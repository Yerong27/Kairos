...
GEMINI_JOB_MODEL=gemini-3.5-flash
GEMINI_JOB_TIMEOUT_SECONDS=120
...
旧版 `.doc` 文件不受支持，请先在 Word 中另存为 `.docx`。
...
Legacy `.doc` files are not supported. Save them as `.docx` in Word first.
