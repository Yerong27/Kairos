...
2. 先展开完整的 job description，等待页面加载完成。
3. 点击 extension 图标。
4. 点击 **Analyze this page**。
5. 显示 **Request sent** 后等待分析完成。
6. 打开 Notion 的 **Kairos-tracker**，查看新生成的职位分析页面。

Gemini 分析和 Notion 写入可能需要一些时间。保持 extension 弹窗打开时，它会显示成功或后端返回的具体错误；最终结果以 Notion 页面为准。
...
```text
当前职位网页
    ↓ Chrome extension 提取最多 20,000 字符
本地 FastAPI 后端
...
分析会保存在 `data/v3_cache/`。缓存 key 包含职位文本、简历、标题和输出语言，所以正常更换简历后会自动产生新结果。
