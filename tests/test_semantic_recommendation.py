...
def test_gemini_deadline_uses_long_timeout_without_spending_a_second_request():
    calls = []
    ...
    assert result["analysis_status"] == "degraded"
    assert "No duplicate retry was sent" in result["evidence_hints"]["error"]
    assert len(calls) == 1
    assert calls[0]["request_options"]["timeout"] == analyzer.JOB_ANALYSIS_TIMEOUT_SECONDS
    assert analyzer.JOB_ANALYSIS_TIMEOUT_SECONDS >= 120
    sleep.assert_not_called()


def test_transient_gemini_service_error_is_retried_once():
    calls = []
    ...
    class FakeModel:
        ...
        def generate_content(self, _prompt, **kwargs):
            calls.append(kwargs)
            if len(calls) == 1:
                raise google_api_exceptions.ServiceUnavailable("temporary overload")
            return SimpleNamespace(
                text=json.dumps(
                    {
                        "job_title": "Engineer",
